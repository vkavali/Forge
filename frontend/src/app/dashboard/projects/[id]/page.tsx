...
import { Project, GenerationJob } from '../../../../lib/types';
import { Button } from '../../../../components/ui/button';
import { Card, CardContent, CardHeader, CardTitle } from '../../../../components/ui/card';
import GenerationStream from '../../../../components/project/GenerationStream';
import { Loader2, FileCode, FileText, ShoppingCart, Rocket, Download } from 'lucide-react';
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <div className="flex gap-2 mt-2"><span className="px-2 py-0.5 bg-blue-600/10 text-blue-400 text-xs rounded-full">{project.category}</span><span className="px-2 py-0.5 bg-gray-800 text-gray-400 text-xs rounded-full">{project.boardId}</span></div>
        </div>
        <div className="flex gap-2">
          ...
          <Button onClick={handleRegenerate} disabled={generating}>{generating ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Rocket className="w-4 h-4 mr-2" />}{generating ? 'Generating...' : 'Regenerate'}</Button>
        </div>
      </div>
      ...
      {tab === 'overview' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Card><CardHeader><CardTitle className="text-lg">Generation History</CardTitle></CardHeader><CardContent>
            {jobs.length === 0 ? <p className="text-gray-500 text-sm">No generations yet</p> : <div className="space-y-2">{jobs.map((job) => (<div key={job.id} className="flex items-center justify-between text-sm"><span className={`px-2 py-0.5 rounded-full text-xs ${job.status === 'COMPLETED' ? 'bg-green-600/10 text-green-400' : job.status === 'FAILED' ? 'bg-red-600/10 text-red-400' : 'bg-yellow-600/10 text-yellow-400'}`}>{job.status}</span><span className="text-gray-500">{new Date(job.createdAt).toLocaleString()}</span></div>))}</div>}
          </CardContent></Card>
        </div>
      )}
      {tab === 'code' && <Card><CardHeader><CardTitle className="text-lg">Generated Code</CardTitle></CardHeader><CardContent>{getArtifact('/code.txt') ? <pre className="bg-gray-950 border border-gray-800 rounded-lg p-4 overflow-x-auto text-sm text-gray-300 whitespace-pre-wrap font-mono">{getArtifact('/code.txt')}</pre> : <p className="text-gray-500">No code generated yet.</p>}</CardContent></Card>}
      {tab === 'docs' && <Card><CardHeader><CardTitle className="text-lg">Documentation</CardTitle></CardHeader><CardContent>{getArtifact('/docs.md') ? <div className="prose prose-invert max-w-none"><ReactMarkdown>{getArtifact('/docs.md')}</ReactMarkdown></div> : <p className="text-gray-500">No documentation generated yet.</p>}</CardContent></Card>}
      {tab === 'bom' && <Card><CardHeader><CardTitle className="text-lg">Bill of Materials</CardTitle></CardHeader><CardContent>{getArtifact('/bom.csv') ? <pre className="bg-gray-950 border border-gray-800 rounded-lg p-4 overflow-x-auto text-sm text-gray-300 whitespace-pre-wrap font-mono">{getArtifact('/bom.csv')}</pre> : <p className="text-gray-500">No BOM generated yet.</p>}</CardContent></Card>}
      {tab === 'deploy' && <Card><CardHeader><CardTitle className="text-lg">Deploy Instructions</CardTitle></CardHeader><CardContent>{getArtifact('/README.md') ? <div className="prose prose-invert max-w-none"><ReactMarkdown>{getArtifact('/README.md')}</ReactMarkdown></div> : <p className="text-gray-500">No deploy instructions generated yet.</p>}</CardContent></Card>}
    </div>
  );
}
